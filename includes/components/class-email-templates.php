...
class KSC_Email_Templates {
    ...
    public static function get_cleaning_invoice_html($service_id) {
        ...
        $transaction_id = get_post_meta($service_id, '_razorpay_payment_id', true);
        
        $invoice_number = 'CLN-' . date('Ymd') . '-' . $service_id;
        $date = date('d M Y');
        ...
        <div style='margin-bottom: 24px;'>
            <h2 style='color: #0f172a; margin: 0 0 8px 0; font-size: 20px;'>Payment Receipt</h2>
            <p style='color: #64748b; margin: 0; font-size: 14px;'>Invoice #{$invoice_number}</p>
        </div>

        <div style='margin-bottom: 32px; color: #334155; line-height: 1.6;'>
            <p style='margin: 0;'><strong>Billed To:</strong> {$customer_name}</p>
            <p style='margin: 0;'><strong>Date:</strong> {$date}</p>
            <p style='margin: 0;'><strong>Transaction ID:</strong> <span style='font-family: monospace; background: #f1f5f9; padding: 2px 6px; border-radius: 4px;'>{$transaction_id}</span></p>
        </div>
        ...
        <p style='color: #475569; margin: 0; line-height: 1.6;'>Your payment has been successfully processed. Our team will contact you shortly to confirm the scheduled date and time for the visit.</p>
        ";
    }
    ...
    public static function get_vendor_invoice_html($vendor_id, $payment_id, $amount, $states_added, $cities_added) {
        ...
        $invoice_number = 'VND-' . date('Ymd') . '-' . substr(md5($payment_id), 0, 6);
        $date = date('d M Y');
        ...
        return "
        <div style='margin-bottom: 24px;'>
            <h2 style='color: #0f172a; margin: 0 0 8px 0; font-size: 20px;'>Tax Invoice / Receipt</h2>
            <p style='color: #64748b; margin: 0; font-size: 14px;'>Invoice #{$invoice_number}</p>
        </div>

        <div style='margin-bottom: 32px; color: #334155; line-height: 1.6;'>
            <p style='margin: 0;'><strong>Billed To:</strong> {$vendor_company}</p>
            <p style='margin: 0;'><strong>Date:</strong> {$date}</p>
            <p style='margin: 0;'><strong>Transaction ID:</strong> <span style='font-family: monospace; background: #f1f5f9; padding: 2px 6px; border-radius: 4px;'>{$payment_id}</span></p>
        </div>
        ...
        <p style='color: #475569; margin: 0; line-height: 1.6;'>Your coverage areas have been successfully expanded in our marketplace. You will now receive leads for the newly added locations.</p>
        ";
    }
}
